...
data_dir=hb_repo_dir+'HB_MCMC/data/lightcurves/original/'
styles_dir='/gpfsm/dnb31/jgbaker/tess/heartbeats/runs/'
...
cmd="python3 -u HB_MCMC_PY --seed=0.082370SEED_N --outname=LABEL --data_style=data_style.json --hb_style=hb_style.json --mcmc_style=mcmc_style.json ${contflag} "
echo $cmd
...
if not dsf.is_file():
    ...
    with open(dsfpath,'w') as sfile:        
        json.dump(dsdict,sfile)
else:
    with open(dsfpath,'r') as sfile:        
        dsdict=json.load(sfile)
ds=dsdict[data_style]
with open(run_dir+'data_style.json','w') as sfile: json.dump(ds,sfile)
...
with open(run_dir+'run_style.json','w') as sfile: json.dump(runpars,sfile)
...
with open(styles_dir+sfname,'r') as sfile:        
    sdict=json.load(sfile)
hbs=sdict[hb_style]
with open(run_dir+'hb_style.json','w') as sfile: json.dump(hbs,sfile)
...
with open(run_dir+'mcmc_style.json','w') as sfile: json.dump(mcs,sfile)

#prep submission script tags
tags='LABEL,RUN_DIR,SEED_N,CONSTRAINT,NODESIZE,NPROCS,TIME,NODES,HB_MCMC_PY'.split(',')
